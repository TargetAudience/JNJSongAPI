...
if($count == 0){
    ...
    if($result['status']==1){
        $code = http_response_code(200);
        echo json_encode(array("response" => 0, "code"=>$code, "message" => "Account Already Registered!"));
        exit();
    }
    if($result['email_otp']==$otp){
        mysqli_query($con,"UPDATE clients set status = 1 where id = $user_id");
        $code = http_response_code(201);
        echo json_encode(array("response" => 1, "code"=>$code, "message" => "Otp Matched! Please login to continue!","user_id"=>$user_id));
        exit(); 
    }else{
        $code = http_response_code(200);
